...
class unpack:
    ...
    def eth_header(self, data):
        ...
        data = {"Destination Mac": mac_formater(destination_mac),
    ...
    def arp_header(self, data):
        storeobj = struct.unpack('2s2s1s1s2s6s4s6s4s', data)
        hardware_type = binascii.hexlify(storeobj[0])
        protocol_type = binascii.hexlify(storeobj[1])
        hardware_size = binascii.hexlify(storeobj[2])
        protocol_size = binascii.hexlify(storeobj[3])
        opcode = binascii.hexlify(storeobj[4])
        source_mac = binascii.hexlify(storeobj[5])
        source_ip = socket.inet_ntoa(storeobj[6])
        dest_mac = binascii.hexlify(storeobj[7])
        dest_ip = socket.inet_ntoa(storeobj[8])
        data = {
